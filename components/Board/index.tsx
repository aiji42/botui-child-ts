...
const Board = () => {
  ...
  useEffect(() => {
    if (loading) return
    const { getSession: { proposals, active } } = data
    if (!active) return
  ...
  return (
    <div style={{ width: '100%', height: '100vh' }}>
      ...
      {!!iframeElement?.contentWindow && initProposals.length && <Commuminator targetWindow={iframeElement.contentWindow} initProposals={initProposals} />}
    </div>
  );
};
